function [w_ij0,w_ki0,gammai,betai,mui,varti,gammak,betak,muk,vartk]=train_net1(train_data,train_label,mini_batch,f1,max_epoch,lr,midnum,decay_epoch,sigma)
%输入：训练数据和标签；输出：训练好的参数
%初始化网络

%% 三层神经网络的创建
innum = 280; %输入神经元个数(输入维数)
%midnum = 200; %隐层神经元个数
outnum = 10; %输出神经元个数
% 权值初始化
...
m={0,0,0,0,0,0};v={0,0,0,0,0,0};delta=cell(1,4);
%% training
